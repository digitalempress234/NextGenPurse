...
export const placeOrderFromCart = async (userId, data) => {
  ...
  const createdOrders = await prisma.$transaction(async (tx) => {
    ...
    if (!cart || cart.items.length === 0) throw createHttpError("Cart is empty", 400);
...
export const getVendorOrders = async (userId, query) => {
  ...
  if (!store) return { items: [], meta: { total: 0, pages: 0, page: 1, limit: 0 } };
  ...
  if (query.status) {
    (whereFilter as any).currentStatus = query.status;
  }
...
export const cancelOrder = async (orderId, userId) => {
  ...
  if (order.userId !== user) throw createHttpError("Unauthorized", 403);
  ...
  await notificationService.createNotification(
    ...
    "order_cancelled",
    { orderId: order.id, orderNumber: order.orderNumber, cancelledAt: new Date() },
    "medium"
  );
...
export const processRefund = async (orderId) => {
  ...
  if (!order.payment) throw createHttpError("Payment record not found", 404);
...
export const updateOrderStatus = async (orderId, newStatus, user) => {
  ...
  if (user.role !== "admin" && order.store.vendorId !== actorId) throw createHttpError("Unauthorized", 403);
