...
export const initializePayment = async (userId, data) => {
  ...
  if (ids.length === 0) throw createHttpError("Provide at least one orderId for payment", 400);
  ...
  if (orders.length === 0) throw createHttpError("No valid orders found", 404);
  if (orders.length !== ids.length) throw createHttpError("Some orders were not found or do not belong to you", 403);
  ...
  if (totalAmount <= 0) throw createHttpError("Total payment amount must be greater than zero", 400);
