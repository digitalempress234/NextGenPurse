...
export const updateDeliveryStatus = async (deliveryId, updateData, user) => {
  ...
  if (user.role !== "admin" && user.role !== "rider") throw new Error("Unauthorized");
  ...
  if (rider) await notificationService.notifyDeliveryAssigned(delivery.order, delivery.order.userId, rider);
  if (status === "delivered") await notificationService.notifyDeliveryCompleted(delivery.order, delivery.order.userId);
