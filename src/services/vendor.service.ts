...
type VendorProfileInput = Partial<{
  firstName: string;
  lastName: string;
  phoneNumber: string;
  state: string;
...
const enforceStep = (user: VendorUser, requiredStep: string) => {
...
const generateOnboardingToken = (user: { id: number; role: string }) => {
  ...
  return jwt.sign({ id: user.id, role: user.role, scope: "vendor_onboarding" }, secret, {
    expiresIn: "7d",
  });
};

export const registerVendor = async (email: string) => {
...
export const resendVendorOTP = async (email: string) => {
  const normalizedEmail = String(email).trim().toLowerCase();
...
export const verifyVendorOTP = async (email: string, otp: string): Promise<{ token: string }> => {
...
export const updateVendorProfile = async (userId: string | number, data: VendorProfileInput) => {
  const id = toIntId(userId, "user id");
  const user = await prisma.user.findUnique({ where: { id } });
  if (!user) throw new Error("User not found");

  enforceStep(user as VendorUser, "verified");
...
export const createStore = async (userId: string | number, data: StoreInput) => {
  const id = toIntId(userId, "user id");
  const user = await prisma.user.findUnique({ where: { id } });
  if (!user) throw new Error("User not found");

  enforceStep(user as VendorUser, "vendor_updated");
...
export const setVendorPassword = async (userId: string | number, password: string) => {
  const id = toIntId(userId, "user id");
  const user = await prisma.user.findUnique({ where: { id } });
  if (!user) throw new Error("User not found");

  enforceStep(user as VendorUser, "store_created");
...
export const uploadVendorDocuments = async (userId: string | number, documents: string[] | string) => {
  const id = toIntId(userId, "user id");
  const user = await prisma.user.findUnique({ where: { id } });
  if (!user) throw new Error("User not found");

  enforceStep(user as VendorUser, "password_set");

  let docs: string[] | string = documents;
...
export const submitForReview = async (userId: string | number) => {
  ...
  enforceStep(user as VendorUser, "documents_uploaded");
