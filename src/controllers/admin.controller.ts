...
const pageArgs = (query: any) => {
  const page = Math.max(1, parseInt(query.page, 10) || 1);
  const limit = Math.min(100, Math.max(1, parseInt(query.limit, 10) || 10));
  return { page, limit, skip: (page - 1) * limit };
};

const searchUserWhere = (search: any) => search
  ? {
      OR: [
...
const pagination = (page: number, limit: number, total: number, key: string) => ({
  currentPage: page,
...
export const getPendingVendors = asyncHandler(async (req: any, res: any) => {
...
export const approveVendor = asyncHandler(async (req: any, res: any) => {
...
export const approveRider = asyncHandler(async (req: any, res: any) => {
  const id = toIntId(req.params.id, "rider id");
  const rider = await prisma.user.findUnique({ where: { id } });
  if (!rider || rider.role !== "rider") {
    res.status(404);
    throw new Error("Rider not found");
  }
  if (rider.onboardingStep !== "under_review") {
    res.status(400);
    throw new Error("Rider is not under review");
  }
  ...
  // TODO: Add notificationService method for rider approval
  res.status(200).json({
    success: true,
    message: "Rider application approved successfully",
    data: { riderId: updated.id, status: "approved", approvedAt: new Date() },
  });
});

export const rejectVendor = asyncHandler(async (req: any, res: any) => {
...
export const rejectRider = asyncHandler(async (req: any, res: any) => {
  const { reason } = req.body;
  if (!reason) {
    res.status(400);
    throw new Error("Rejection reason is required");
  ...
  // TODO: Add notificationService method for rider rejection
  res.status(200).json({
    success: true,
    message: "Rider application rejected",
    data: { riderId: updated.id, status: "rejected", reason, rejectedAt: new Date() },
  });
});

export const getAllVendors = asyncHandler(async (req: any, res: any) => {
  const { page, limit, skip } = pageArgs(req.query);
  const where: any = { role: "vendor", ...searchUserWhere(req.query.search) };
...
export const getAllCustomers = asyncHandler(async (req: any, res: any) => {
  const { page, limit, skip } = pageArgs(req.query);
...
export const getAllRiders = asyncHandler(async (req: any, res: any) => {
  const { page, limit, skip } = pageArgs(req.query);
...
export const updateUserStatus = asyncHandler(async (req: any, res: any) => {
...
export const getDashboardStats = asyncHandler(async (_req: any, res: any) => {
  const [
    totalUsers,
    totalVendors,
    totalCustomers,
