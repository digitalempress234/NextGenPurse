...
const enforceStep = (user, requiredStep) => {
    if (user.onboardingStep !== requiredStep)
        throw createHttpError(`Complete ${requiredStep} step first`, 400);
};
...
export const registerRider = async (email) => {
...
export const resendRiderOTP = async (email) => {
    const normalizedEmail = String(email).trim().toLowerCase();
...
export const verifyRiderOTP = async (email, otp) => {
...
export const getRiderProfile = async (userId) => {
    ...
    if (!rider)
        throw createHttpError("Rider not found", 404);
    return rider;
};
export const setRiderPassword = async (userId, password) => {
    const id = toIntId(userId, "user id");
...
export const updateRiderProfile = async (userId, data) => {
    ...
    const { vehicleType, vehicleNumber, firstName, lastName, gender, dateOfBirth, email, phoneNumber, city, } = data;
    const updateData = {
    ...
    if (vehicleType)
        profileUpdateData.vehicleType = vehicleType;
    if (vehicleNumber)
        profileUpdateData.vehicleNumber = vehicleNumber;
...
export const setRiderBusinessDetails = async (userId, data) => {
    const id = toIntId(userId, "user id");
    const user = await prisma.user.findUnique({ where: { id } });
    if (!user)
        throw createHttpError("User not found", 404);
