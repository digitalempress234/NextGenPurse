...
export const acceptOffer = async (riderId, offerId) => {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (!offer || offer.riderId !== rId)
            throw createHttpError("Offer not found", 404);
        if (offer.status !== "pending")
            throw createHttpError("Offer is no longer available", 400);
...
export const verifyPickup = async (riderId, deliveryId, code) => {
    ...
    if (!delivery || delivery.riderId !== rId)
        throw createHttpError("Delivery not found", 404);
    if (delivery.pickupCode !== code)
        throw createHttpError("Invalid pickup code", 400);
...
export const verifyDelivery = async (riderId, deliveryId, code) => {
    ...
    if (delivery.deliveryCode !== code)
        throw createHttpError("Invalid delivery code", 400);
    return prisma.$transaction(async (tx) => {
...
//# sourceMappingURL=riderLogistics.service.js.map