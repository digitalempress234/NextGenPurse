...
const resolveCategoryIds = async ({ category, subCategory }, existingProduct = null) => {
    ...
    if (subCategoryProvided) {
        if (!subCategory) {
            ...
            if (!subCategoryDoc)
                throw createHttpError("Sub-category not found", 404);
            subCategoryId = subCategoryDoc.id;
            if (!categoryProvided && !categoryId && subCategoryDoc.parentId)
                categoryId = subCategoryDoc.parentId;
    ...
    if (categoryProvided && !subCategoryProvided && subCategoryId) {
        ...
        if (subCategoryDoc?.parentId && subCategoryDoc.parentId !== categoryId)
            subCategoryId = null;
    }
    return {
        categoryId,
...
export const getProducts = async (query) => {
    ...
    if (!where)
        return emptyPage(pageNumber, limitNumber);
...
export const updateProduct = async (userId, productId, data) => {
    ...
    if (!existing)
        throw createHttpError("Product not found", 404);
    const { categoryId, subCategoryId, shouldSetCategory, shouldSetSubCategory } = await resolveCategoryIds({ category: data.category, subCategory: data.subCategory }, existing);
    const updateData = {};
    ...
    if (data.expiryDate !== undefined)
        updateData.expiryDate = data.expiryDate ? new Date(String(data.expiryDate)) : null;
