...
export const getAllVendors = asyncHandler(async (req, res) => {
    ...
    if (req.query.status === "approved")
        where.onboardingStep = "approved";
    if (req.query.status === "pending")
        where.onboardingStep = "under_review";
...
export const updateUserStatus = asyncHandler(async (req, res) => {
    ...
    await notificationService.createNotification(user.id, title, message, "system_announcement", { reason: req.body.reason, updatedBy: currentUserId }, "high");
    res.status(200).json({
        success: true,
...
export const getDashboardStats = asyncHandler(async (_req, res) => {
    const [totalUsers, totalVendors, totalCustomers, pendingVendors, totalOrders, recentOrders, orderGroups, revenue,] = await Promise.all([
        prisma.user.count(),
